...
from .config import is_training as tflearn_is_training
...
class TrainOp(object):
    ...
    def _train(self, training_step, snapshot_epoch, snapshot_step,
        ...
        tflearn_is_training(True, session=self.session)
        ...
        if snapshot and self.val_feed_dict:
            tflearn_is_training(False, session=self.session)
...
def evaluate_flow(session, ops_to_evaluate, dataflow):
        ...
        tflearn_is_training(False, session)
        dataflow.reset()
        dataflow.start()
